...
export async function POST(req: Request) {
    try {
        ...
        const result = await UserModel.updateOne({ email: normalizedEmail }, { $set: { role } })
        ...
        return NextResponse.json({ success: false, message: "Failed to update role. Please try again." }, { status: 500 })
    }
}
